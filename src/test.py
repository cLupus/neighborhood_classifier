...
def run():
    ...
    target = targets[0]
    path = folder + target + extension
    normalized_path = folder + normalized + target + extension

    roi = RegionsOfInterest(path, normalizing_path=normalized_path, mode='gauss')
    roi.set_aggregate(True)
    net = ClassificationNet(roi, ['soil'], neigborhood_size=3, targets_background_ration=[1, 1], hidden_ratio=0.98)
    net.neural_net.randomize()

    # soil = net.data_set.data.

    net.set_trainer(learning_rate=0.2, verbose=True, momentum=0.1, weight_decay=0.01)
    net.train_network(max_epochs=10, verbose=True)
    net.save('../soil.nn')
    # #print(net)
